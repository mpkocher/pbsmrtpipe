"""Report Model. This is taken from pbreports."""
#!/usr/bin/env python
import abc
import logging
import json
...
class Attribute(BaseReportElement):
    ...
    def __repr__(self):
        _d = dict(k=self.__class__.__name__,
                  ...
                  v=self.value,
                  i=self.id)
        return "<{k} id={i} name=\"{n}\" v={v} >".format(**_d)
...
class AttributeGroup(BaseReportElement):

    def __init__(self, id_, title, attributes=()):
        super(AttributeGroup, self).__init__(id_)
        # Not Sure if title should be necessary?
        self.title = title
...
class Table(BaseReportElement):
    ...
    def append_data(self, column_index, item):
        """
        This should be depreciated in favor of `add_data_by_column_id`.
...
class Report(BaseReportElement):
    ...
    def __repr__(self):
        _d = dict(k=self.__class__.__name__,
                  ...
                  t=len(self.tables))
        return "<{k} nattributes={a} nplotgroups={p} ntables={t} >".format(**_d)
    ...
    def to_json(self):
        ...
        try:
            s = json.dumps(self.to_dict(), cls=NumpyJsonEncoder, indent=4, sort_keys=True)
